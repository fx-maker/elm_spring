...
import com.neusoft.feign.PointsFeignClient;
...
public class OrdersServiceImpl implements OrdersService {
    ...
    public int payOrders(Integer orderId) {
        ...
        CommonResult walletResult = walletFeignClient.pay(order.getUserId(), order.getOrderTotal());
        if (walletResult.getCode() == 200) {
            int updated = ordersMapper.updateOrderState(orderId);
            if (updated > 0) {
                pointsFeignClient.addPoints(
                    order.getUserId(),
                    "ORDER",
                    order.getOrderTotal(),
                    orderId,
